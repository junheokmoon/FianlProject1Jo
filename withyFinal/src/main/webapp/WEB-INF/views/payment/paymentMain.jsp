...
<body>
	...
	$(".pay").click(function(event) {
		...
		alert("버튼이 눌렸따!_");
			var paymentUserNo=parseInt(${userInfo.userNo}); // 유저 정보 가져오기
			var paymentTicketCode=${ticketInfo.ticketCode}; // 티켓 번호 가져오기
			var paymentUsepoint = $("#usePointInput").val; // 사용한 포인트 가져오기
			var pg=$(this).attr("id"); // 결제 대행사 정보 가져오기
			
			var IMP=window.IMP;
			IMP.init("imp20277585"); // 고객사 식별 코드 (내 결제 코드)
			
			//주문번호 - 주문테이블에서 제공된 값 사용
			var paymentNo = paymentUserNo+new Date().getTime();
		...
		alert("변수 만들었따!_");
			...
			$.ajax({
				...
				data: JSON.stringify({"paymentNo":paymentNo, "paymentUserNo":paymentUserNo, "paymentTicketCode":paymentTicketCode
				, "paymentUsepoint":paymentUsepoint,"paymentStatus":1 ,"paymentPrice":totalPrice}),
				dataType: "text",
				success: function(result) {
					if(result=="ok") {
						...
						IMP.request_pay({
							...
							amount : totalPrice,
							//결제창에 보여질 제품명
							name: ${ticketInfo.ottkindDTO.ottName}+${ticketInfo.ticketMonth }+"개월 티켓",
							//결제 사용자의 이메일 주소 
				            buyer_email: ${userInfo.userEmail},
				            buyer_name: ${userInfo.userName},//결제 사용자 이름
				            buyer_tel: ${userInfo.userPhone},//결제 사용자 전화번호
