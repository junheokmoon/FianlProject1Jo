...
public class InquiryServiceImpl implements InquiryService{
	...
	@Override
	public void modifyInquiryAnswer(InquiryDTO inquiry) {
	...
	@Override
	public InquiryDTO getInquiryAndUser(int inquiryNo) {
		InquiryDTO inquiry=inquiryDAO.selectInquiryAndUser(inquiryNo);
		return inquiry;
	}
	...
	//페이징 처리 관련 정보를 Map 객체의 엔트리로 추가하여 반환하는 메소드 
	@Override
	public Map<String, Object> getInquiryListTen(int pageNum) {
		//noticeDTO 테이블에 저장된 모든 행(게시글)의 갯수를 반환받아 저장
		int totalSize=inquiryDAO.selectInquiryCount();
		
		Pager pager=new Pager(pageNum, totalSize, 10, 5); //한 페이지에 5개 게시글 갯수 저장, 한 블럭에 출력될 페이지 번호를 5개 저장 
		...
		List<InquiryDTO> inquiryList=inquiryDAO.selectInquiryListTen(pageMap);
	...
	@Override
	public List<InquiryDTO> getInquiryListFive() {
		return inquiryDAO.selectInquiryListFive();
	}
